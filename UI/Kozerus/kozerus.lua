--[[
	Kozerus UI Library by Kozenomenon - https://github.com/Kozenomenon/RBX_Pub/tree/main/UI/Kozerus
    A modified version of Cerebus, found here: https://github.com/Jxereas/UI-Libraries
    Changes made: 
...
local cg,sg,hmm,hf,gnc,snc,gconn,ghui,syngui,scb
do -- executor normalization
	cg = syn_context_get or (syn and syn.get_thread_identity) or getidentity or getthreadidentity or getthreadcontext
	sg = syn_context_set or (syn and syn.set_thread_identity) or setidentity or setthreadidentity or setthreadcontext
	hmm = hookmetamethod
	hf = hookfunction or hookfunc or detour_function or replaceclosure
	gnc = getnamecallmethod
	snc = setnamecallmethod
	gconn = getconnections or get_signal_connections
	ghui = gethui or hiddenUI or get_hidden_gui
	syngui = not is_sirhurt_closure and syn and syn.protect_gui

    -- optional, only here for kick msg if the above fail to resolve
    scb = setclipboard or toclipboard
end

local Players = game:GetService("Players")
local player = Players.LocalPlayer

-- kick if executor support check fails
if not (cg and sg and hmm and hf and gnc and snc and gconn and (ghui or syngui)) then
    if scb then
        scb('https://github.com/Kozenomenon/RBX_Pub/tree/main/UI/Kozerus')
    end
	player:Kick('Your executor does not meet the requirements for Kozerus UI!'..(scb and ' Kozerus url added to clipboard' or ''))
end

for _, connection in pairs(gconn(player.Idled)) do
...
if not _G.JxereasExistingHooks.GuiDetectionBypass then
    ...
    local old
    old = hf(ContentProvider.PreloadAsync, function(self, tbl, cb)
    ...
    old = hmm(game, "__namecall", function(self, ...)
        ...
        if self == ContentProvider and (method == "PreloadAsync" or method == "preloadAsync") then
            ...
            local err
            task.spawn(function()
                snc(method) --different thread, different namecall method
...
local function createOriginalElements()
	local function createWindow()
		...
		headingSeperator.BackgroundColor3 = Color3.fromRGB(39, 131, 131)
	...
	local function createTab()
		...
		tabSeperator.BackgroundColor3 = Color3.fromRGB(4, 255, 255)
	...
	local function createPage(includeBottomFrame)
		...
		leftScrollingFrame.Size = UDim2.new(0.5, -5, includeBottomFrame and 0.75 or 1, includeBottomFrame and -5 or 0)
		...
		rightScrollingFrame.Size = UDim2.new(0.5, -5, includeBottomFrame and 0.75 or 1, includeBottomFrame and -5 or 0)
		...
		if includeBottomFrame then
			local bottomScrollingFrame = Instance.new("ScrollingFrame")
			local bottomScrollingFrameList = Instance.new("UIListLayout")

			bottomScrollingFrame.Name = "BottomScrollingFrame"
			bottomScrollingFrame.Active = true
			bottomScrollingFrame.AnchorPoint = Vector2.new(1, 1)
			bottomScrollingFrame.BackgroundColor3 = Color3.fromRGB(31, 31, 43)
			bottomScrollingFrame.BackgroundTransparency = 1.000
			bottomScrollingFrame.Position = UDim2.new(1, 0, 1, 0)
			bottomScrollingFrame.Size = UDim2.new(1, -5, 0.25, 0)
			bottomScrollingFrame.CanvasSize = UDim2.fromScale(0,0)
			bottomScrollingFrame.ScrollBarThickness = 0
			bottomScrollingFrame.Parent = page
			
			bottomScrollingFrameList.Name = "BottomScrollingFrameList"
			bottomScrollingFrameList.Padding = UDim.new(0,7)
			bottomScrollingFrameList.HorizontalAlignment = Enum.HorizontalAlignment.Center
			bottomScrollingFrameList.Parent = bottomScrollingFrame
		end
	...
	local function createSection()
		...
		headingSeperator.BackgroundColor3 = Color3.fromRGB(33, 163, 163)
		headingSeperator.BorderSizePixel = 0
		headingSeperator.Position = UDim2.new(0, 0, 1, 0)
	...
	local function createTitle()
		...
		designGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(31, 31, 43)), ColorSequenceKeypoint.new(0.50, Color3.fromRGB(33, 163, 163)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(31, 31, 43))}
	...
	local function createToggle()
		...
		toggleImage.BackgroundColor3 = Color3.fromRGB(0, 255, 255)
	...
	local function createButton()
		...
		buttonCircle.BackgroundColor3 = Color3.fromRGB(0, 255, 255)
...
function Library.new(...):table
	...
	if ghui then
		if gconn then
			local succ_dc,dc = pcall(function()
				local conns = gconn(game.DescendantAdded)
				local cnt = 0
				if conns then
					cnt = #conns
	...
	minimizedShortBarOriginialSize = Vector2.new(heading.AbsoluteSize.X / 6 * 2.5, heading.AbsoluteSize.Y)
...
function tabHandler:UpdateFrameScale():nil
	if self.includeBottomFrame then
		local left_right_size,bottom_size
		if self.bottomMinimized then
			local bottom_frame = self.ElementToParentChildren:FindFirstChild('BottomScrollingFrame')
			if bottom_frame then
				for _,child in ipairs(bottom_frame:GetChildren()) do
					if child and child:IsA('Frame') then
						local heading = child:FindFirstChild('Heading')
						if heading then
							local y_offset = heading.Size.Y.Offset
							bottom_size = UDim2.new(1,-5,0,y_offset)
							left_right_size = UDim2.new(0.5,-5,1,-1*(y_offset+5))
							break
						end
					end
				end
			...
			left_right_size = UDim2.new(0.5, -5, 1-self.bottomFrameYScale, -5)
			bottom_size = UDim2.new(1,-5,self.bottomFrameYScale,0)
		end
		for _, scrollingFrame in ipairs(self.ElementToParentChildren:GetChildren()) do
			if scrollingFrame.Name:match('Bottom') and bottom_size then
				scrollingFrame.Size = bottom_size
			elseif left_right_size then
				scrollingFrame.Size = left_right_size
			end
		end
	end
end

function tabHandler:MinimizeBottom():nil
	self.bottomMinimized = true
	self:UpdateFrameScale()
end

function tabHandler:MaximizeBottom():nil
	self.bottomMinimized = false
	self:UpdateFrameScale()
end
...
function tabHandler:Section(...):table
	...
	local function getShorterScrollingFrame()
		...
		for _, scrollingFrame in ipairs(self.ElementToParentChildren:GetChildren()) do
			if scrollingFrame.Name:match('Bottom') then continue end
			local list = scrollingFrame:FindFirstChildWhichIsA("UIListLayout")
	...
	if params.parentDesignation and type(params.parentDesignation)=='string' then
		...
		elseif pd=='right' then
			parent = self.ElementToParentChildren.RightScrollingFrame
		elseif pd=='bottom' then
			parent = self.ElementToParentChildren:FindFirstChild('BottomScrollingFrame')
		end
	end
...
function labelHandler:updateSize()
	local lbltxt = self.Instance.LabelBackground.LabelText
	local padding_offsets = lbltxt.LabelTextPadding.PaddingTop.Offset + lbltxt.LabelTextPadding.PaddingBottom.Offset + self.Instance.LabelPadding.PaddingTop.Offset + self.Instance.LabelPadding.PaddingBottom.Offset + self.Instance.LabelBackground.LabelBackgroundPadding.PaddingTop.Offset + self.Instance.LabelBackground.LabelBackgroundPadding.PaddingBottom.Offset
	if lbltxt.RichText then
		self.Instance.Size = UDim2.new(1, 0, 0, math.max(10, lbltxt.TextBounds.Y)  + padding_offsets)
	else
		local textParams = Instance.new("GetTextBoundsParams")
		textParams.Text = lbltxt.ContentText or lbltxt.Text or "N/A"
		textParams.Font = lbltxt.FontFace
		textParams.Size = lbltxt.TextSize
		textParams.Width = lbltxt.AbsoluteSize.X - lbltxt.LabelTextPadding.PaddingLeft.Offset - lbltxt.LabelTextPadding.PaddingRight.Offset
		self.Instance.Size = UDim2.new(1,0,0,TextService:GetTextBoundsAsync(textParams).Y + padding_offsets)
	end
end
...
function elementHandler:Dropdown(...): table
	...
	dropdown.SetExpanded = function(self,expanded:boolean):nil
		if expanded~=dropdown.IsExpanded then
			onDropdownClicked()
		end
	end
...
function elementHandler:Slider(...): table
	...
	local orig_back_color = sliderBar.BackgroundColor3
	local disabled_back_color = Color3.fromRGB(orig_back_color.R/2,orig_back_color.G/2,orig_back_color.B/2)
	slider.SetEnabled = function(self,state:boolean):nil
		enabled = state
		sliderInstance.TextGrouping.NumberText.TextEditable = enabled
		sliderBar.BackgroundColor3 = enabled and orig_back_color or disabled_back_color
	end
...
function elementHandler:ColorWheel(...): table
	...
	colorWheel.Set = function(self,color:Color3):nil
